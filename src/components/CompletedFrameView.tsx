...
export const CompletedFrameView: React.FC<CompletedFrameViewProps> = ({
  ...
  }, [countdown, isPaused, onRedirectToLanding]);
  ...
  return (
    ...
    >
      {/* 5-Second Auto-Redirect Notice */}
      <div
        className={`flex items-center justify-between px-3.5 py-2 rounded-2xl text-xs font-bold border transition ${
          isFelt
      ...
      >
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            title={isPaused ? 'Resume countdown' : 'Pause auto-redirect'}
            className="p-1 rounded-lg bg-black/20 hover:bg-black/30 transition text-emerald-300 cursor-pointer"
          >
            {isPaused ? <Play className="w-3.5 h-3.5" /> : <Pause className="w-3.5 h-3.5" />}
          </button>
          <button
            type="button"
            onClick={onRedirectToLanding}
            className="px-2 py-0.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-[11px] font-black flex items-center gap-1 cursor-pointer transition shadow-xs"
          >
            <span>Go Now</span>
